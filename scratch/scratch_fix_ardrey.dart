...
void main() async {
    ...
    for (final field in section.fields) {
      if (field.type == SurveyFieldType.repeatableTable) {
        ...
        if (field.key == 'smokers') {
          ...
          fullPayload[field.key] = [{'name': 'Dummy ${field.label}', 'age': 30, 'gender': 'Male', 'reason': 'Dummy Reason', 'frequency': 'Everyday', 'cause': 'Dummy Cause', 'admitted': 'Yes', 'bp': '120/80', 'family_position': 'Head'}];
        }
  ...
  fullPayload['family_members'] = familyMembers;

  final payloadJsonStr = jsonEncode(fullPayload).replaceAll("'", "''");
  final familyMembersJsonStr = jsonEncode(familyMembers).replaceAll("'", "''");

  final updateQuery = '''
    UPDATE household_assessments 
    SET 
      payload = ''' + "'" + payloadJsonStr + "'" + '''::jsonb,
      family_members = ''' + "'" + familyMembersJsonStr + "'" + '''::jsonb,
      updated_at = NOW()
