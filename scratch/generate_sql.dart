...
// Copy the demo data methods from app_controller here since they are private
List<HealthSubmission> getDemoSubmissions() {
...
INSERT INTO public.household_assessments (
  ...
  '${s.respondentName.replaceAll("'", "''")}',
  ${s.respondentAge ?? 'NULL'},
  '${s.address.replaceAll("'", "''")}',
  ${s.familyMembersCount},
  '$familyMembersStr'::jsonb,
  '$healthProblemsStr'::text[],
  '${s.vaccinationStatus.replaceAll("'", "''")}',
  '${s.waterSanitation.replaceAll("'", "''")}',
  '${s.nutritionalStatus.replaceAll("'", "''")}',
  '$concernsStr'::text[],
  ${s.consentGiven},
  '${s.notes.replaceAll("'", "''")}',
  '$payloadStr'::jsonb
