...
void main() {
  ...
  String inject(String source, String keyword, String tag, {int skipOccurrences = 0}) {
    final xmlGap = r'(?:<[^>]+>)*';
    final flexKeyword = keyword.split('').map((char) {
  ...
  void injectCell(XmlElement tc, String text) {
    var p = tc.children.whereType<XmlElement>().where((e) => e.name.local == 'p').firstOrNull;
    if (p == null) {
  ...
  if (tables.isNotEmpty) {
    ...
    final rows = demoTable.descendants.whereType<XmlElement>().where((e) => e.name.local == 'tr').toList();
    // Headers are in rows 0,1,2. Data rows start at index 3.
    // There are 13 rows for data.
    int rowIndex = 0;
    for (int i = 3; i < rows.length && rowIndex < 13; i++) {
      final cells = rows[i].descendants.whereType<XmlElement>().where((e) => e.name.local == 'tc').toList();
  ...
  if (anthroTable != null) {
    final rows = anthroTable.descendants.whereType<XmlElement>().where((e) => e.name.local == 'tr').toList();
  ...
  for (final t in tables) {
    if (t.innerText.contains('Food taken') && t.innerText.contains('BREAKFAST')) {
      recallTable = t;
      break;
    }
  }
  if (recallTable != null) {
    final rows = recallTable.descendants.whereType<XmlElement>().where((e) => e.name.local == 'tr').toList();
    // Rows 1 to 6 are the meal rows
    final mealRows = ['breakfast', 'snack1', 'lunch', 'snack2', 'dinner', 'midnight_snack'];
    for (int i = 0; i < mealRows.length; i++) {
      final rIdx = i + 1; // start from row 1
      if (rIdx < rows.length) {
        final cells = rows[rIdx].descendants.whereType<XmlElement>().where((e) => e.name.local == 'tc').toList();
        if (cells.length >= 3) {
  ...
  for (final file in archive) {
    if (file.name == 'word/document.xml') {
      newArchive.addFile(ArchiveFile.string('word/document.xml', doc.toXmlString(pretty: false)));
    } else {
